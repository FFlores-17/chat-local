 ...
 * ───────────────────────────────────────── */
void *review_outbox(void *arg) { //Bug durante Caso 1: se cambio la funcion debido a un problema con las rutas y la funcion LIST
    (void)arg;

    char  path[256];
    char  line[MAX_LINE_LEN];
    char  pending[64][MAX_LINE_LEN];
    int   n_pending;
    FILE *f;
    DIR  *dir;
    struct dirent *entry;
    ...
    while (g_running) {
        ...
        /* Escanear TODOS los archivos en outbox/, no solo usuarios registrados */
        dir = opendir(OUTBOX_DIR);
        if (dir) {
            while ((entry = readdir(dir)) != NULL) {
                if (entry->d_name[0] == '.') continue;

                snprintf(path, sizeof(path), "%s%s", OUTBOX_DIR, entry->d_name);

                /* Buscar el offset guardado para este archivo */
                long offset = 0;
                int user_idx = -1;
                ...
                for (int i = 0; i < MAX_USERS; i++) {
                    if (!g_user_table.users[i].active) continue;
                    char expected[256];
                    snprintf(expected, sizeof(expected), "client_%d.txt",
                             g_user_table.users[i].pid);
                    if (strcmp(entry->d_name, expected) == 0) {
                        offset    = g_user_table.users[i].outbox_offset;
                        user_idx  = i;
                        break;
                    }
                }
                ...
                n_pending = 0;
                while (fgets(line, sizeof(line), f) != NULL && n_pending < 64) {
                ...
                /* Actualizar offset si el usuario ya está registrado */
                if (user_idx >= 0) {
                    pthread_mutex_lock(&g_user_table.mutex);
                    g_user_table.users[user_idx].outbox_offset = new_offset;
                    pthread_mutex_unlock(&g_user_table.mutex);
                }
                ...
                /* Después del LOGIN el usuario ya está registrado — actualizar offset */
                if (user_idx < 0) {
                    pthread_mutex_lock(&g_user_table.mutex);
                    for (int i = 0; i < MAX_USERS; i++) {
                        if (!g_user_table.users[i].active) continue;
                        ...
                        snprintf(expected, sizeof(expected), "client_%d.txt",
                                 g_user_table.users[i].pid);
                        if (strcmp(entry->d_name, expected) == 0) {
                            g_user_table.users[i].outbox_offset = new_offset;
                            break;
                        }
                    }
                    pthread_mutex_unlock(&g_user_table.mutex);
